...
public class Service {
    ...
    protected List<Train> createTrainList() {
    ...
    protected List<Station> createRoutes(int routeId) {
    ...
    public void checkPlaceState(List<Train> trainsForUser, int coachNumber) {
        for (Train train : trainsForUser) {
            for (Place place : train.getCoaches().get(coachNumber).getPlaces()) {
                if (place.getState() && (place.getStartStation() != null
                        && place.getEndStation() != null)) {
                    place.setState(false);
                }
            }
        }
